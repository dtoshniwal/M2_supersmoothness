...
homologyDimensionTable(ZZ,ZZ,List):= (a,b,L) ->(
    --M := splineModule(L_0, L_1, L_2, L_3); -- this does not account for specified supersmoothness!
...
cellularComplex(List,ZZ) := ChainComplex => opts -> (F,degD) -> (
    degD = apply(#F, i->degD);
    cellularComplex(F, degD,opts)
)

cellularComplex(List,List) := ChainComplex => opts -> (F,degD) -> (
    L := cellularModules(F,degD,opts);
    fullmodulelist := L_0;
    intC := L_1;
    S := L_2;
...
------------------------------------------------
cellularModules = method(Options =>{
    symbol Homogenize => true, 
    symbol VariableName => getSymbol "t",
    symbol CoefficientRing => QQ,
    symbol BaseRing => null})
------------------------------------------------
---This method computes the moduels in the cellular chain complex
---of a polyhedral complex with coefficients in a polynomial
---ring, modulo the boundary
------------------------------------------------
...
--- to its boundary.
--------------------------------------------------
cellularModules(List,List) := ChainComplex => opts -> (F,degD) -> (
    d := #(first F) -1;
    S := createSplineRing(d,opts);
    ...
    scan(#intC-1,i->(
        if #(intC_(i+1))==0 then(
            ...
            newMod = directSum apply(#(intC_(i+1)),j->(
                ...
                return sub(module ideal (hvar^((degDAll_(i+1))_j)), S)
            ));
        );
        fullmodulelist = append(fullmodulelist,newMod)
    ));
    {fullmodulelist, intC, S}
)
...
idealsComplex(List,List,ZZ,ZZ):=ChainComplex => opts -> (V,F,r,degD)->(
    ...
    re := apply(#E, i -> r);
    rv := apply(#V, i -> r);
    idealsComplex(V,F,{E,re,rv},degD,opts)
)
...
--V: vertex list
--F: facet list
--Er: {list of comdim-1 facets, list/value of smoothness across codim-1 facets, list/value of smoothness across codim-d facets} 
--degD: degree deficits on facets
------------------------------------------
--Outputs: The mixed degree Schenck-Stillman complex of ideals
------------------------------------------
...
idealsComplex(List,List,List,List):=ChainComplex => opts -> (V,F,Er,degD)->(
    L := idealsModules(V,F,Er,degD,opts);
    fullmodulelist := L_0;
    intC := L_1;
    S := L_2;
    --defining the chain complex
...
------------------------------------------
--This function computes the mixed degree generalization
-- of the modules in the Schenck-Stillman chain complex
-- of ideals for a simplicial or polyhedral complex
------------------------------------------
...
--Outputs: The mixed degree Schenck-Stillman complex of ideals
------------------------------------------
idealsModules(List,List,List,List):=List => opts -> (V,F,Er,degD)->(
    E := Er_0;
    re := Er_1;
    rv := Er_2;
    try #E==#re then () else ( re = apply(#E, i -> re); );
    try #V==#rv then () else ( rv = apply(#V, i -> rv); );
    d := #(first V);
    S := createSplineRing(d,opts);
    varlist := vars S;
    --list of interior faces in order of increasing codimension--
    intC := getInteriorFaces(F);
    re = arrangeSmoothnessIndices(F, E, re);
    --get degree deficits on all faces in increasing codimension
    degDAll := getAllDegreeDeficits(F, degD);
    --list of forms defining codim 1 interior faces
    intformslist :=formsList(V,intC_1,re,degDAll_1,opts);
    --list of modules which will define chain complex--
    fullEmodulelist := apply(#(intC_1),i->(
        e := (intC_1)_i;
        ev := subsets(e, 1);
        vertideals := apply(ev,vi->sub((ideal apply(d, k->varlist_(0,k)-V_(vi_0)_k*varlist_(0,d)))^(rv_(vi_0)+1),S));
        sub(module intersect (sub(ideal (intformslist_i),S), intersect vertideals), S)
    ));
    fullmodulelist := apply(#intC,i->(
        if #(intC_i)==0 then(
            ...
            if i==1 then(
                newMod = sub(directSum (fullEmodulelist), S);
            ) else (
                newMod = directSum apply(intC_i,e->(
                    CE := positions(intC_1,f->subsetL(e,f));
                    if #(CE)==0 then(
                        tmp := image matrix{{0_S}};
                    )else(
                        tmp = sub(sum (fullEmodulelist_CE), S);
                    );
                    tmp
                ));
            );
...
splineComplex(List,List,ZZ,ZZ):=ChainComplex => opts -> (V,F,r,degD)->(
    ...
    re := apply(#E, i -> r);
    rv := apply(#V, i -> r);
    splineComplex(V,F,{E,re,rv},degD,opts)
)
...
splineComplex(List,List,List,List):=ChainComplex => opts -> (V,F,Er,degD)->(
    C := cellularModules(F,degD,opts);
    I := idealsModules(V,F,Er,degD,opts);
    fullmodulelistC := C_0;
    fullmodulelistI := I_0;
    intC := C_1;
    S := C_2;
    T := I_2;
    fullmodulelist := apply(#fullmodulelistC, i-> sub(fullmodulelistC_i,S)/sub(fullmodulelistI_i,S));
    --defining the chain complex
    CCSS :=chainComplex(reverse apply(#intC-1, c-> (
        inducedMap(fullmodulelist_(c+1),fullmodulelist_c,sub(simpBoundary(intC_c,intC_(c+1)),S))
    )))
